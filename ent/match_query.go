...
func (mq *MatchQuery) QueryMentor() *MentorQuery {
...
func (mq *MatchQuery) QueryMenti() *MentiQuery {
...
func (mq *MatchQuery) QueryMessages() *MessageQuery {
...
func (mq *MatchQuery) Clone() *MatchQuery {
	...
	return &MatchQuery{
		...
		withMentor:   mq.withMentor.Clone(),
		withMenti:    mq.withMenti.Clone(),
...
func (mq *MatchQuery) WithMentor(opts ...func(*MentorQuery)) *MatchQuery {
	query := (&MentorClient{config: mq.config}).Query()
...
func (mq *MatchQuery) WithMenti(opts ...func(*MentiQuery)) *MatchQuery {
	query := (&MentiClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MatchQuery) WithMessages(opts ...func(*MessageQuery)) *MatchQuery {
	query := (&MessageClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MatchQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Match, error) {
	...
	_spec.Node.Schema = mq.schemaConfig.Match
	...
	if query := mq.withMentor; query != nil {
		if err := mq.loadMentor(ctx, query, nodes, nil,
			func(n *Match, e *Mentor) { n.Edges.Mentor = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withMenti; query != nil {
		if err := mq.loadMenti(ctx, query, nodes, nil,
			func(n *Match, e *Menti) { n.Edges.Menti = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withMessages; query != nil {
		if err := mq.loadMessages(ctx, query, nodes,
			func(n *Match) { n.Edges.Messages = []*Message{} },
			func(n *Match, e *Message) { n.Edges.Messages = append(n.Edges.Messages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MatchQuery) loadMentor(ctx context.Context, query *MentorQuery, nodes []*Match, init func(*Match), assign func(*Match, *Mentor)) error {
...
func (mq *MatchQuery) loadMenti(ctx context.Context, query *MentiQuery, nodes []*Match, init func(*Match), assign func(*Match, *Menti)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Match)
	for i := range nodes {
		fk := nodes[i].MentiID
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "menti_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MatchQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Match, init func(*Match), assign func(*Match, *Message)) error {
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(match.MessagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.message_match
		if fk == nil {
			return fmt.Errorf(`foreign-key "message_match" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "message_match" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (mq *MatchQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withMentor != nil {
			_spec.Node.AddColumnOnce(match.FieldMentorID)
		}
		if mq.withMenti != nil {
			_spec.Node.AddColumnOnce(match.FieldMentiID)
		}
	}
...
func (mq *MatchQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(mq.schemaConfig.Match)
	ctx = internal.NewSchemaConfigContext(ctx, mq.schemaConfig)

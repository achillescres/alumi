...
// was not loaded in eager-loading, or loaded but was not found.
func (e MessageEdges) MatchOrErr() (*Match, error) {
	if e.Match != nil {
		return e.Match, nil
...
func (*Message) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case message.ForeignKeys[0]: // message_from
			values[i] = new(sql.NullInt64)
		case message.ForeignKeys[1]: // message_match
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case message.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field message_from", value)
			} else if value.Valid {
				m.message_from = new(int)
				*m.message_from = int(value.Int64)
			}
		case message.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field message_match", value)
			} else if value.Valid {
				m.message_match = new(int)
				*m.message_match = int(value.Int64)
			}
		default:
...
// QueryMatch queries the "match" edge of the Message entity.
func (m *Message) QueryMatch() *MatchQuery {
	return NewMessageClient(m.config).QueryMatch(m)
}

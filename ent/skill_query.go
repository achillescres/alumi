...
func (sq *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	_spec.Node.Schema = sq.schemaConfig.Skill
...
func (sq *SkillQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *User)) error {
	...
	for i := range nodes {
		if nodes[i].skill_user == nil {
			continue
		}
		fk := *nodes[i].skill_user
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "skill_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SkillQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(sq.schemaConfig.Skill)
	ctx = internal.NewSchemaConfigContext(ctx, sq.schemaConfig)

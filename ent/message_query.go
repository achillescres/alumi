...
func (mq *MessageQuery) QueryMatch() *MatchQuery {
...
func (mq *MessageQuery) WithMatch(opts ...func(*MatchQuery)) *MessageQuery {
	query := (&MatchClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if mq.withFrom != nil || mq.withMatch != nil {
		withFKs = true
	}
	if withFKs {
	...
	_spec.Node.Schema = mq.schemaConfig.Message
	...
	if query := mq.withMatch; query != nil {
		if err := mq.loadMatch(ctx, query, nodes, nil,
			func(n *Message, e *Match) { n.Edges.Match = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadFrom(ctx context.Context, query *UserQuery, nodes []*Message, init func(*Message), assign func(*Message, *User)) error {
	...
	for i := range nodes {
		if nodes[i].message_from == nil {
			continue
		}
		fk := *nodes[i].message_from
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "message_from" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) loadMatch(ctx context.Context, query *MatchQuery, nodes []*Message, init func(*Message), assign func(*Message, *Match)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].message_match == nil {
			continue
		}
		fk := *nodes[i].message_match
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "message_match" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(mq.schemaConfig.Message)
	ctx = internal.NewSchemaConfigContext(ctx, mq.schemaConfig)

...
func (mc *MessageCreate) SetMatchID(id int) *MessageCreate {
	mc.mutation.SetMatchID(id)
	return mc
}

// SetMatch sets the "match" edge to the Match entity.
func (mc *MessageCreate) SetMatch(m *Match) *MessageCreate {
	return mc.SetMatchID(m.ID)
}
...
func (mc *MessageCreate) check() error {
	...
	if _, ok := mc.mutation.FromID(); !ok {
		return &ValidationError{Name: "from", err: errors.New(`ent: missing required edge "Message.from"`)}
	}
	if _, ok := mc.mutation.MatchID(); !ok {
		return &ValidationError{Name: "match", err: errors.New(`ent: missing required edge "Message.match"`)}
	}
	return nil
}
...
func (mc *MessageCreate) createSpec() (*Message, *sqlgraph.CreateSpec) {
	...
	_spec.Schema = mc.schemaConfig.Message
	...
	if nodes := mc.mutation.FromIDs(); len(nodes) > 0 {
		...
		_node.message_from = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	if nodes := mc.mutation.MatchIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: false,
			Table:   message.MatchTable,
			Columns: []string{message.MatchColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
